...
if (!empty($search_start_delivery_date))
    $tmp_date_start = str_replace('/', '-', $search_start_delivery_date);
if (!empty($search_end_delivery_date))
    $tmp_date_end = str_replace('/', '-', $search_end_delivery_date);
...
if (getDolGlobalString('SHIP2BILL_LIST_LENGTH'))
    $conf->liste_limit = getDolGlobalString('SHIP2BILL_LIST_LENGTH');
$offset = 0;
...
if (isset($_REQUEST['subCreateBill'])) {
    ...
    if (empty($TExpedition)) {
        ...
        //Si le nombre de facture est négatif alors on n'affiche pas le message car la fonction est en erreur
        if ($nbFacture >= 0) {
...
if ($action == 'remove_file') {
    ...
    if ($ret)
        ...
        setEventMessage($langs->trans("ErrorFailToDeleteFile", GETPOST('urlfile', 'alpha')), 'errors');
    $action = '';
...
if ($search_ref_client)
    $sql .= natural_search('c.ref_client', $search_ref_client);
if ($search_ref_cde)
    $sql .= natural_search('c.ref', $search_ref_cde);
if ($search_ref_liv)
...
if ($search_order_statut != -4 && $search_order_statut != -1 && $search_order_statut != '')
    $sql .= " AND c.fk_statut = " . intval($search_order_statut);
...
if ($search_ref_exp)
    $sql2 .= natural_search('e.ref', $search_ref_exp);
if ($search_ref_client)
    $sql2 .= natural_search('c.ref_client', $search_ref_client);
if ($search_ref_cde)
    $sql2 .= natural_search('c.ref', $search_ref_cde);
if ($search_ref_liv)
    $sql2 .= natural_search('l.ref', $search_ref_liv);
if ($search_societe)
    $sql2 .= natural_search('s.nom', $search_societe);
if ($search_status != -1 && $search_status != '')
    $sql2 .= " AND e.fk_statut = " . $search_status;
if (!empty($search_start_delivery_date) && !empty($search_end_delivery_date)) {
...
if ($resql) {
    ...
    if ($search_ref_exp)
        $param .= "&amp;search_ref_exp=" . $search_ref_exp;
    if ($search_ref_client)
        $param .= "&amp;search_ref_client=" . $search_ref_client;
    if ($search_ref_cde)
        $param .= "&amp;search_ref_cde=" . $search_ref_cde;
    if ($search_ref_liv)
    ...
    if ($search_start_delivery_date)
        $param .= "&amp;search_start_delivery_date=" . $search_start_delivery_date;
    if ($search_end_delivery_date)
        $param .= "&amp;search_end_delivery_date=" . $search_end_delivery_date;
    ...
    while ($i < min($num, $limit)) {
        ...
        if (!empty($conf->livraison_bon->enabled) || !empty($conf->delivery_note->enabled)) {
            ...
            if (!empty($shipment->linkedObjects['delivery']))
